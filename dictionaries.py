    ...
    for def_item in def_section_list:
        ...
        for sseq_group in sseq_outer:
            for sseq_inner_list in sseq_group:
                if isinstance(sseq_inner_list, list) and len(sseq_inner_list) > 1:
                    ...
                    if sense_type in ['sense', 'bs']:
                        if 'dt' in sense_content:
                            ...
                            for dt_element in dt_array:
                                if isinstance(dt_element, list) and dt_element[0] == 'text':
                                    raw_def_text = dt_element[1]
                                    cleaned_def = clean_definition_text(raw_def_text)
                                    if cleaned_def:
                                        current_defs_to_add.append(cleaned_def)
                        elif 'sense' in sense_content and 'dt' in sense_content['sense']:
                        ...
                        for p_sense_item in sense_content:
                            if isinstance(p_sense_item, list) and len(p_sense_item) > 1 and p_sense_item[0] == 'sense':
...
def process_api_response_to_string(api_response_data):
    ...
    for entry_index, entry in enumerate(api_response_data):
        if not isinstance(entry, dict):
            output_lines.append(f"Skipping invalid entry at index {entry_index}: {entry}")
            continue

        word_id = entry.get('meta', {}).get('id', 'N/A')
        hwi_data = entry.get('hwi', {})
        headword = hwi_data.get('hw', word_id.split(':')[0]) if isinstance(hwi_data, dict) else word_id.split(':')[0]
        ...
        if definitions:
            ...
            output_lines.append("  No definitions found for this entry structure.")
        
        output_lines.append("") # Add a blank line for separation between entries
